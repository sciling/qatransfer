...
def _test(config):
    test_data = read_data(config, config.test_name, True)
    ...
    if config.model_name == 'basic':
        ThisEvaluator = MultiGPUF1Evaluator
    elif config.model_name in ['basic-class', 'basic-generate']:
        ThisEvaluator = MultiGPUClassificationAccuracyEvaluator
    evaluator = ThisEvaluator(config, models, tensor_dict=models[0].tensor_dict if config.vis else None)
