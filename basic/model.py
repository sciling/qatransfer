...
class Model(object):
    ...
    def _build_loss(self):
        config = self.config
        is_basic = config.model_name == 'basic'
        is_class = config.model_name == 'basic-class'

        N, M, JX = tuple([tf.shape(self.x)[i] for i in range(3)])
        JQ = tf.shape(self.q)[1]
    ...
    def get_var_list(self):
        if self.config.freeze_span_modelling_layer:
            prefixs = ["output/g0", "output/classifier"]
        elif self.config.freeze_att_layer:
            prefixs = ["output"]
        elif self.config.freeze_phrase_layer:
            prefixs = ["output", "main"]
        else:
            prefixs = [""]
        vars_ = []
        for prefix in prefixs:
            vars_ += tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, prefix)
        return vars_ #self.var_list
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        is_basic = config.model_name == 'basic'
        is_class = config.model_name == 'basic-class'
        is_squad = config.data_dir.startswith('data/squad')

        if supervised and is_basic:
        ...
        if supervised and is_class:

            C = 3 if config.data_dir.startswith('data/snli') else 2
            y0 = np.zeros([N, M, C], dtype='float32')
        ...
        for i, cqi in enumerate(batch.data['cq']):
            for j, cqij in enumerate(cqi):
                if j == config.max_num_sents: break
